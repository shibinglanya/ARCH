...
function! SignDetectTask(start, middle, end, timer) abort
	let l:bufnr = bufnr()
	...
	if [beg, end] != getbufvar(l:bufnr, 'sd_range', [0, 0])
		...
		if !(beg < end && beg > 0 && end <= line('$')) | return | endif
		let s:place_flag = 0
...
func! s:searchpairpos_aux(start, middle, end, flags)
    return searchpair(a:start, a:middle, a:end, a:flags,
        \"synIDattr(synID(line('.'), col('.'), 0), 'name') =~? '" .
        \'string\|Rainbow_lv\d_r\d\|comment' .
        \"'")
endf
