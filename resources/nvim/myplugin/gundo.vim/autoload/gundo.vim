...
function! s:GundoMapGraph()"{{{
    ...
    nnoremap <script> <silent> <buffer> l             :call <sid>GundoRevert()<CR>
    ...
    nnoremap <script> <silent> <buffer> <Esc>         :call <sid>GundoClose()<CR>
...
function! s:update_timer.clone(winnr) abort
    call setwinvar(a:winnr, 'gundo_update_id', 
                \ getwinvar(a:winnr, 'gundo_update_id', -1) + 1)
    let l:other_timer                 = copy(self)
    let l:other_timer.id              = getwinvar(a:winnr, 'gundo_update_id', -1)
    let l:other_timer.winnr           = a:winnr
    function! l:other_timer.task(timer) abort
        if self.id == getwinvar(self.winnr, 'gundo_update_id', -1)
          call s:GundoRenderPreview()
        endif
    endfunction
...
function! s:GundoMove(direction) range"{{{
    ...
    if g:gundo_auto_preview == 1
        call timer_start(100, s:update_timer.clone(winnr()).task, {'repeat': 1})
    endif
endfunction"}}}
