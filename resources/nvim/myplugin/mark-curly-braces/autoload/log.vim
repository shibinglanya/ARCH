...
let s:hlchunk_log_file = get(g:, 'hlchunk_log_file', '/tmp/mark-curly-braces.log')
...
func! s:writefile(...) abort
	if !empty(trim(get(s:, 'hlchunk_log_file', '')))
...
function! log#detector_print(event)
	call s:writefile(printf('{触发事件:%s}', a:event))
endfunction
