...
void applyrules(Client *c) {
  ...
  // XGetTextProperty(dpy, c->win,&tp,XInternAtom(dpy, "_NET_WM_NAME", False));

  // FILE *fp = fopen("/tmp/dwm.log", "a+");
  // fprintf(fp, "%s", tp.value);
  // fclose(fp);
  ...
  for (i = 0; i < LENGTH(rules); i++) {
    ...
    //  (r->class && !strcmp(r->class, tp.value)))
    {
      c->isfloating = r->isfloating;
      if (r->isfullscreen) {
        c->isrfullscreen = r->isfullscreen;
        for (int i = 0; i < LENGTH(tags); ++i) {
          if ((r->tags >> i) & 1) {
            selmon->pertag->showbars[i + 1] = 0;
            updatebarpos(selmon);
            XMoveResizeWindow(dpy, selmon->barwin, selmon->wx, selmon->by,
                              selmon->ww, bh);
            selmon->pertag->ltidxs[i + 1][0] = &layouts[2];
            selmon->pertag->ltidxs[i + 1][1] = &layouts[2];
            break;
          }
        }
      }
    ...
    if (text[i] == '^' && !isCode) {
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          buf[7] = '\0';
          drw_clr_create_(drw, &drw->scheme[ColFg], buf);
          i += 7;
        } else if (text[i] == 'b') {
          char buf[8];
          memcpy(buf, (char *)text + i + 1, 7);
          buf[7] = '\0';
          drw_clr_create_(drw, &drw->scheme[ColBg], buf);
          i += 7;
        } else if (text[i] == 'C') {
          int c = atoi(text + ++i);
          drw_clr_create_(drw, &drw->scheme[ColFg], termcolor[c]);
        } else if (text[i] == 'B') {
          int c = atoi(text + ++i);
          drw_clr_create_(drw, &drw->scheme[ColBg], termcolor[c]);
        } else if (text[i] == 'd') {
          drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
          drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
        } else if (text[i] == 'w') {
...
void movemouse(const Arg *arg) {
  ...
  if (!(c = selmon->sel))
    return;
  if (c->isfullscreen ||
      c->isrfullscreen) /* no support moving fullscreen windows by mouse */
    return;
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
...
void resizemouse(const Arg *arg) {
  ...
  if (!(c = selmon->sel))
    return;
  if (c->isfullscreen ||
      c->isrfullscreen) /* no support resizing fullscreen windows by mouse */
    return;
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
